...
class MiniImageNetGenerator(object):
    ...
    def untar_mini(self):
        ...
        self.train_path = self.imagenet_dir+'/train/'
        self.val_path = self.imagenet_dir+'/val/'
        self.test_path = self.imagenet_dir+'/test/'
        ...
        if not os.path.exists(self.processed_img_dir+'train/'):
            os.makedirs(self.processed_img_dir+'train/')
        if not os.path.exists(self.processed_img_dir+'val/'):
            os.makedirs(self.processed_img_dir+'val/')
        if not os.path.exists(self.processed_img_dir+'test/'):
            os.makedirs(self.processed_img_dir+'test/')

        
        if not os.path.exists(self.train_path):
            print ("Image-Net/train path not exist: ", self.train_path)
        
        
        if not os.path.exists(self.val_path):
            print ("Image-Net/val path not exist: ", self.val_path)
            
        
        if not os.path.exists(self.test_path):
            print ("Image-Net/val path not exist: ", self.test_path)
            
        
        for idx, keys in enumerate(self.mini_keys):
            if  os.path.exists(self.train_path+keys):
                copytree(self.train_path+keys, self.processed_img_dir+'train/'+keys)
                print('copy Train' + keys)
                
                
            if  os.path.exists(self.val_path+keys):
                copytree(self.val_path+keys, self.processed_img_dir+'val/'+keys)
                print('copy Val' + keys)
            
            if  os.path.exists(self.test_path+keys):
                copytree(self.test_path+keys, self.processed_img_dir+'test/'+keys)
                print('copy Test' + keys)
